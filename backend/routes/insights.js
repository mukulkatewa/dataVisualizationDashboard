 ...
 * @desc    Get all insights with pagination
 * @access  Public
...
/**
 * @route   GET /api/insights/filters/options
 * @desc    Get unique values for all filter fields (for dropdown population)
 * @access  Public
 */
router.get('/filters/options', async (req, res) => {
    try {
        ...
        // Extract SWOT categories (these are typically part of the topic or insight field)
        // Common SWOT terms to look for
        ...
        res.status(200).json({
            ...
            data: {
                ...
                sectors,
                regions,
                pestles,      // PEST filter (Political, Economic, Social, Technological)
                sources,
                countries,
                cities,
                swot: swotCategories
...
/**
 * @route   GET /api/insights/filter
 * @desc    Filter insights by multiple criteria
 * @access  Public
 * @query   end_year, topic, sector, region, pestle, source, country, city, 
 *          start_year, intensity_min, intensity_max, likelihood_min, likelihood_max,
 *          relevance_min, relevance_max, search, page, limit
 */
router.get('/filter', async (req, res) => {
    try {
        ...
        // SWOT filter - search in title and insight fields
        if (swot) {
            filter.$or = [
                { title: { $regex: swot, $options: 'i' } },
                { insight: { $regex: swot, $options: 'i' } }
            ];
        }
        ...
        // Full-text search on title and insight
        if (search) {
            const searchFilter = {
                $or: [
        ...
        res.status(500).json({
            success: false,
            error: 'Server error while filtering insights'
        });
    }
});
...
router.get('/:id', async (req, res) => {
    try {
        ...
        res.status(500).json({
            success: false,
            error: 'Server error while fetching insight'
        });
    }
});
